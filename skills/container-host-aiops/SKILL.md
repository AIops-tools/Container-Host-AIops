...
description: >
  ...
  Preview — governed Docker/Portainer container-host operations with a built-in governance harness (audit, policy, token budget, undo, risk-tiers). Mock-validated only, not verified against a live Docker daemon or Portainer server.
installer:
  kind: uv
...
compatibility: >
  Standalone, self-governed Docker + Portainer container-host operations (preview). The governance harness (audit, policy, token/runaway budget, undo, risk-tiers) is bundled in the package — no external skill-family dependency. Multi-platform by construction (a platform registry); a per-target 'platform' field (docker / portainer) selects the API shape.
  ...
  PREVIEW: mock-validated only; the Docker Engine + Portainer API paths are modelled from the public API shape and need live verification. Community-maintained; not affiliated with or endorsed by Docker/Portainer — trademarks belong to their owners.
---
