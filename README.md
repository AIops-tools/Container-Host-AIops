...
- **MCP server** (`container-host-aiops mcp` or `container-host-aiops-mcp`): **36 tools** (28 read, 8 write), every one wrapped with the bundled `@governed_tool` harness.
...
## Capability matrix (36 MCP tools)
...
**Preview — mock-validated only, not verified against a live Docker daemon, Portainer, or Podman server.** The API paths are modelled from the public API shape and need live verification. `container-host-aiops doctor` is the fastest live check.
