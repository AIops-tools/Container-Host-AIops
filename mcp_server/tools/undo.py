...
the SAME governance path as any other call — audited (over MCP and the CLI
alike) and budget-checked. ``undo_apply`` is itself governed; whether the
inverse should run is the caller's decision, as with any write.
...
def _origin_target(rec: dict) -> Optional[str]:
    ...
    usually does not exist there; when two hosts both have a container named
    ``web`` the inverse succeeds on the wrong one, silently.

    Caught live on 2026-08-03: a ``stop_container`` recorded against a Podman
    target replayed against a Portainer target and 404'd.
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    The inverse runs through its own governed tool, so it is audited on the same
    path as any other call. Pass dry_run=True to preview the inverse call without
    ...
    return {
        ...
        "note": (
            ""
            if effect_verified
            else "The original write lost its response, so it was never confirmed to "
            "have taken effect. This inverse ran regardless; report the CURRENT "
            "server state, not a restore that may have had nothing to restore."
        ),
    }
