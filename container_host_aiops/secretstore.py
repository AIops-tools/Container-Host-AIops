"""Encrypted secret storage for container-host-aiops.

Secrets (Portainer API tokens) are NEVER written to disk in plaintext. They
live in ``~/.container-host-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``CONTAINER_HOST_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "container-host-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "CONTAINER_HOST_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``CONTAINER_HOST_<TARGET>_PASSWORD``; ``prefix='CONTAINER_HOST_'`` and
