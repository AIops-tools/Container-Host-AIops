"""``container-host-aiops undo`` — list recorded undo tokens and apply them.
...
def undo_apply_cmd(
    ...
    if dry_run:
        # Already routed through the governed twin, but the refusal was being
        # swallowed: an error dict left ``wouldApply`` empty and still printed a
        # green "inverse: ?" banner with exit 0. Route the render through the
        # helper so a refused preview teaches and exits non-zero instead.
