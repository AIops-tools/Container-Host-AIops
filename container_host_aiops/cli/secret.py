"""``container-host-aiops secret`` — manage the encrypted credential store.

Secrets (Portainer API tokens) are stored in ``~/.container-host-aiops/secrets.enc``
...
from container_host_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_list() -> None:
    ...
    if not names:
        console.print(
            "[yellow]No secrets stored yet. Add one: container-host-aiops secret set <name>[/]"
        )
        return
...
def secret_rotate_password() -> None:
    ...
    console.print(
        "[green]✓ Master password rotated. "
        "Update CONTAINER_HOST_AIOPS_MASTER_PASSWORD.[/]"
    )
