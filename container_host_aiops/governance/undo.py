...
``{skill, tool, params}``) into ``~/.container-host-aiops/undo.db`` and hands back an
...
_log = logging.getLogger("container-host-aiops.undo")
...
class UndoStore:
    ...
    def record(
        ...
        *,
        ...
        ``effect_verified=False`` marks a token whose *original* write lost its
        response: the change is probably live but unconfirmed. The token stays
        listable and appliable — an unconfirmed change is exactly when a
        rollback is most wanted — but the flag travels with it so an operator is
        never told a state was restored that was never reached.
        """
        if not self._ok:
            return None
        ...
        try:
            ...
            conn.execute(
                ...
                "undo_params, orig_params, status, workflow_id, note, effect_verified) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'recorded', ?, ?, ?)",
                (
                    undo_id,
