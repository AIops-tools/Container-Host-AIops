...
Set ``CONTAINER_HOST_READ_ONLY=1`` (or ``true``/``yes``/``on``) and this tool can only
read. Enforcement is deliberately two-layered:

  1. :func:`~container_host_aiops.governance.decorators.governed_tool` refuses any
...
READ_ONLY_ENV = "CONTAINER_HOST_READ_ONLY"
