...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``CONTAINER_HOST_RUNAWAY_MAX`` times within ``CONTAINER_HOST_RUNAWAY_WINDOW_SEC`` trips a
...
from container_host_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing container-host-specific; the decorator wraps it into a
