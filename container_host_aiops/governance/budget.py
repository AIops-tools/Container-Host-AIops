...
1. **Hard ceilings (opt-in via env).** Total tool calls and cumulative
   wall-time per process. ``CONTAINER_HOST_MAX_TOOL_CALLS`` / ``CONTAINER_HOST_MAX_TOOL_SECONDS``.
   Unset (the default) means no ceiling.

2. **Runaway breaker (on by default, disable with ``CONTAINER_HOST_RUNAWAY_MAX=0``).**
   The same ``(tool, params)`` called more than ``CONTAINER_HOST_RUNAWAY_MAX`` times
   within ``CONTAINER_HOST_RUNAWAY_WINDOW_SEC`` trips a short cooldown. Catches a tight
...
class BudgetTracker:
    ...
    mid-session (e.g. an operator sets ``CONTAINER_HOST_RUNAWAY_MAX`` higher) without a
    ...
    def _max_calls() -> int | None:
        return _env_int("CONTAINER_HOST_MAX_TOOL_CALLS", None)

    @staticmethod
    def _max_seconds() -> int | None:
        return _env_int("CONTAINER_HOST_MAX_TOOL_SECONDS", None)

    @staticmethod
    def _runaway_max() -> int:
        # 0 or negative disables the runaway breaker.
        return _env_int("CONTAINER_HOST_RUNAWAY_MAX", _DEFAULT_RUNAWAY_MAX) or 0

    @staticmethod
    def _runaway_window() -> int:
        return _env_int("CONTAINER_HOST_RUNAWAY_WINDOW_SEC", _DEFAULT_RUNAWAY_WINDOW_SEC) or 0
        ...
        with self._lock:
            ...
            if max_calls is not None and self._state.total_calls >= max_calls:
                raise BudgetExceeded(
                    ...
                    f"(CONTAINER_HOST_MAX_TOOL_CALLS={max_calls}). Stop and summarize what "
            ...
            if max_seconds is not None and self._state.total_seconds >= max_seconds:
                raise BudgetExceeded(
                    ...
                    f"(CONTAINER_HOST_MAX_TOOL_SECONDS={max_seconds}). Stop and report "
            ...
            if runaway_max > 0 and window > 0:
                ...
                if len(dq) >= runaway_max:
                    raise BudgetExceeded(
                        ...
                        f"(Raise CONTAINER_HOST_RUNAWAY_MAX if this is genuinely intended.)",
                        rule="budget_runaway",
                    )
                dq.append(now)
