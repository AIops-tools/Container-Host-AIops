...
Responsibilities:
  ...
  3. Post-log: write audit record to ``~/.container-host-aiops/audit.db``
...
from container_host_aiops.governance.audit import detect_agent, get_engine
from container_host_aiops.governance.budget import BudgetExceeded, get_budget
from container_host_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from container_host_aiops.governance.patterns import PatternMatch, get_pattern_engine
from container_host_aiops.governance.policy import PolicyResult, get_policy_engine
from container_host_aiops.governance.readonly import is_read_only, read_only_denial
from container_host_aiops.governance.sanitize import sanitize

_log = logging.getLogger("container-host-aiops.decorators")
    ...
    *,
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.container-host-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("CONTAINER_HOST_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("CONTAINER_HOST_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (CONTAINER_HOST_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set CONTAINER_HOST_AUDIT_APPROVED_BY to the authorizing human (and "
            f"CONTAINER_HOST_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`container_host_aiops.governance.outcome`). The change may be live,
    ...
    if not descriptor:
        return
    try:
        from container_host_aiops.governance.undo import get_undo_store
...
def _infer_skill(func: Any) -> str:
    ...
    ``container_host_aiops.ops.jobs`` → ``container-host-aiops``
    ``mcp_server.server`` → ``container-host-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("container_host_aiops") or module.startswith("mcp_server"):
        return "container-host-aiops"
    return "unknown"
