"""Policy engine — rule-based access control for container-host MCP tools.

Rules are loaded from ``~/.container-host-aiops/rules.yaml`` with hot-reload on file change.
"""
...
_log = logging.getLogger("container-host-aiops.policy")
...
class PolicyEngine:
    ...
    def check_allowed(
        ...
        *,
        ...
        if os.environ.get("CONTAINER_HOST_POLICY_DISABLED") == "1":
        ...
        if not tiers:
            if not self._rules and risk_level in ("high", "critical"):
                return TierDecision(
                    ...
                    reason=(
                        f"No rules file at {self._path} — high/critical operations require a named approver (CONTAINER_HOST_AUDIT_APPROVED_BY) by default. Author rules.yaml "
