"""Unified audit logging engine — all container-host skills write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.container-host-aiops/audit.db``.
...
_log = logging.getLogger("container-host-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring CONTAINER_HOST_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the CONTAINER_HOST_AIOPS_HOME refactor
