...
``container_host_aiops.governance.*`` and binds state to an isolated home via
``CONTAINER_HOST_AIOPS_HOME``. Every governed function under test is SYNTHETIC and
...
import container_host_aiops.governance.budget as budget_mod
import container_host_aiops.governance.decorators as dec_mod
...
from container_host_aiops.governance.budget import BudgetExceeded
...
def dec_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CONTAINER_HOST_MAX_TOOL_CALLS", raising=False)
    monkeypatch.delenv("CONTAINER_HOST_MAX_TOOL_SECONDS", raising=False)
    _reset()
    yield tmp_path
    _reset()
...
def test_budget_exceeded_propagates_and_audits(dec_home, monkeypatch):
    monkeypatch.setenv("CONTAINER_HOST_MAX_TOOL_CALLS", "0")
...
def test_policy_disabled_marks_status_bypassed(dec_home, monkeypatch):
    monkeypatch.setenv("CONTAINER_HOST_POLICY_DISABLED", "1")
    _reset()
