...
def test_undo_apply_replays_against_the_target_the_write_ran_on(gov_home):
    ...
    exist there; two hosts with a container named ``web`` and the inverse
    succeeds on the wrong one, silently. Caught live on 2026-08-03: a stop
    ...
    uid = undo_mod.get_undo_store().record(
        ...
        orig_params={"value": "v1", "target": "podman"},
    )
    result = gov.undo_apply(undo_id=uid)
    assert result["applied"] is True
    assert _TARGETS == ["podman"]
...
def test_undo_apply_prefers_an_explicit_caller_target(gov_home):
    ...
    gov.undo_apply(undo_id=uid, target="portainer")
    assert _TARGETS == ["portainer"]
