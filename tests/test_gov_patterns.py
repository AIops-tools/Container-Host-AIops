...
``container_host_aiops.governance.patterns`` and binds state to an isolated home via
``CONTAINER_HOST_AIOPS_HOME``. No ops / cli / connection / mcp_server imports.
...
import container_host_aiops.governance.patterns as patterns_mod
from container_host_aiops.governance.patterns import (
    PatternEngine,
    get_pattern_engine,
...
def test_match_armed(patterns_dir):
    _write_pattern(patterns_dir, skill="container-host-aiops", tool="reindex_table")
    eng = PatternEngine(patterns_dir)
    m = eng.match(skill="container-host-aiops", tool="reindex_table", target="db1")
...
def test_match_none_when_no_action_matches(patterns_dir):
    ...
    eng = PatternEngine(patterns_dir)
    assert eng.match(skill="container-host-aiops", tool="other", target="db1") is None
...
def test_rate_limit_hourly_cap(patterns_dir):
    ...
    eng = PatternEngine(patterns_dir)
    assert eng.match("container-host-aiops", "reindex_table", "db1").armed is True
    assert eng.match("container-host-aiops", "reindex_table", "db1").armed is True
    third = eng.match("container-host-aiops", "reindex_table", "db1")
    assert third.armed is False
    assert "hourly cap" in third.reason
    # A different target has its own budget.
    assert eng.match("container-host-aiops", "reindex_table", "db2").armed is True
...
def test_rate_limit_daily_cap(patterns_dir):
    ...
    eng = PatternEngine(patterns_dir)
    assert eng.match("container-host-aiops", "reindex_table", "db1").armed is True
    second = eng.match("container-host-aiops", "reindex_table", "db1")
...
def test_circuit_breaker_trips_after_threshold_failures(patterns_dir):
    ...
    m = eng.match("container-host-aiops", "reindex_table", "db1")
    assert m.armed is False
...
def test_hot_reload_on_new_file(patterns_dir):
    ...
    eng = PatternEngine(patterns_dir)
    assert eng.match("container-host-aiops", "reindex_table", "db1") is not None
    ...
    m = eng.match("container-host-aiops", "vacuum_table", "db1")
    assert m is not None
...
def test_hot_reload_on_mtime_change(patterns_dir):
    ...
    assert eng.match("container-host-aiops", "analyze_table", "db1") is not None
...
def test_hot_reload_on_dir_deletion(patterns_dir):
    ...
    assert eng.match("container-host-aiops", "reindex_table", "db1") is None
    assert eng.loaded_patterns() == []
...
def test_reset_state_clears_counters(patterns_dir):
    ...
    eng = PatternEngine(patterns_dir)
    eng.match("container-host-aiops", "reindex_table", "db1")
