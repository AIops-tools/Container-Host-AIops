...
throwaway home (``CONTAINER_HOST_AIOPS_HOME``) and assert that the rows
compliance evidence is built from actually land on disk, and that the
harness records but never authorizes — a high-risk write runs with no
approver set and is still audited.
"""
...
from container_host_aiops.governance import (
    capture_prior_state,
    governed_tool,
    mark_unknown,
)
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="prod-host")
...
def test_high_risk_runs_without_an_approver_and_is_audited(gov_home):
    ...
    result = _drop_widget(name="w2", target="prod-host")
...
def test_an_approver_is_still_recorded_when_supplied(gov_home, monkeypatch):
    ...
    monkeypatch.setenv("CONTAINER_HOST_AUDIT_APPROVED_BY", "ops-alice")
    result = _drop_widget(name="w3", target="prod-host")
    assert result["status"] == "dropped"
...
class _FakeConn:
    """Fake Docker connection: canned inspect JSON + recorded POSTs."""

    def __init__(self, inspect: dict) -> None:
        self._inspect = inspect
        self.posts: list[str] = []

    def docker_get(self, path: str, params: dict | None = None) -> dict:
        return self._inspect

    def docker_post(self, path: str, params: dict | None = None, json: dict | None = None) -> dict:
...
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: stop_container must
    capture the container's prior running state before stopping and persist
    the start-undo on disk."""
    from mcp_server.tools import writes as gov

    conn = _FakeConn({"Id": "abc123", "Name": "/web", "State": {"Running": True}})
    monkeypatch.setattr(gov, "_get_connection", lambda target=None: conn)

    result = gov.stop_container(container_id="abc123")
    assert result["priorState"] == {"running": True}
    assert result.get("_undo_id")
    assert conn.posts == ["/containers/abc123/stop"]

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "start_container"
    assert json.loads(undo[0]["undo_params"]) == {"container_id": "abc123"}

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["stop_container"]
...
def test_self_lockout_refusal_writes_no_undo_and_audits_as_error(gov_home, monkeypatch):
    """The Portainer self-lockout guard, end-to-end through the governed tool.

    This is the path the CLI takes too (``cli/manage.py`` delegates to these
    same governed functions), so proving it here proves BOTH entry points. The
    refusal must reach the caller as an error payload, must fire no Docker call,
    and must leave NO undo token behind — a refused write changed nothing, so a
    recorded inverse would be a lie.
    """
    from mcp_server.tools import writes as gov

    conn = _FakeConn({"Id": "p1", "Name": "/portainer",
                      "Config": {"Image": "portainer/portainer-ce:2.19.4"}})
    conn.target = SimpleNamespace(platform="portainer", port=9443)
    monkeypatch.setattr(gov, "_get_connection", lambda target=None: conn)

    result = gov.stop_container(container_id="p1")
    assert "Refusing to stop" in result["error"]
    assert "docker" in result["error"]  # the remedy survived the 300-char cap
    assert conn.posts == []  # nothing was sent to the host
    assert result.get("_undo_id") is None

    # The undo store was never even opened — nothing was reversed because
    # nothing happened.
    assert not (gov_home / "undo.db").exists() or not _rows(gov_home / "undo.db", "undo_log")
    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["stop_container"]
