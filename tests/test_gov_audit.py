...
``container_host_aiops.governance.audit`` and binds state to an isolated home via
...
from container_host_aiops.governance.audit import (
    AuditEngine,
    _current_user,
    _safe_json,
    detect_agent,
...
def test_log_writes_row_with_all_columns(audit_home):
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row["skill"] == "container-host-aiops"
...
def test_query_filters(audit_home):
    eng = AuditEngine(audit_home / "audit.db")
    eng.log(skill="container-host-aiops", tool="a", status="ok", workflow_id="w1")
    eng.log(skill="container-host-aiops", tool="b", status="error", workflow_id="w2")
    eng.log(skill="other", tool="a", status="ok", workflow_id="w1")

    assert len(eng.query()) == 3
    assert len(eng.query(skill="container-host-aiops")) == 2
...
def test_stats_aggregation(audit_home):
    eng = AuditEngine(audit_home / "audit.db")
    eng.log(skill="container-host-aiops", tool="a", status="ok")
    eng.log(skill="container-host-aiops", tool="b", status="error")
    ...
    assert stats["by_skill"] == {"container-host-aiops": 2, "other": 1}
