...
from container_host_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "remove_container" in dropped and "stop_container" in dropped
        assert "list_containers" not in dropped and "container_logs" not in dropped
        ...
        assert "list_containers" in remaining, "reads must still be exposed"
    finally:
        registry.clear()
