...
### Changed
- **The ClawHub bundle plugin moved from `@aiops-tools/container-host-aiops` to
  `@zw008/container-host-aiops`**, matching the publisher the skill has always been under.
  ...
  `openclaw plugins install clawhub:@zw008/container-host-aiops`. Nothing about the Python
...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/container-host-aiops`. States the `uvx`
...
- **Installable as a Claude Code plugin.** `.claude-plugin/plugin.json` plus a
  root `.mcp.json` make this repo a plugin, so `/plugin install container-host-aiops@aiops-tools`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/container-host-aiops`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [container-host-aiops, uvx]` — either one suffices — with every variable kept
